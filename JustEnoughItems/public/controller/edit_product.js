...
export function addEventListeners(){
    ...
    Element.formEditProduct.addEventListener('submit', async e=>{
        ...
        if(errors){
            if(errors.name) {Element.formEditProductError.name.innerHTML = errors.name}
            if(errors.price) {Element.formEditProductError.price.innerHTML = errors.price}
            if(errors.summary) {Element.formEditProductError.summary.innerHTML = errors.summary}
...
export async function deleteProduct(docId, imageName){
    try{
        ...
        Util.popupInfo("Deleted", `${docId} has been deleted`)
        //update web browser
        const card = document.getElementById(`card-${docId}`)
        card.remove()

        Util.popupInfo("Deleted", "item deleted")
    }catch(e){
        if(Constant.DEV) console.log(e)
        Util.popupInfo("deleteProductError", JSON.stringify(e))
    }
}
